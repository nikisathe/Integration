...
const student_connection_AMRT = mysql.createConnection({
    host: 'localhost',
    user: 'root', 
    password: '', 
    database: "AMRT12"
...
student_connection_AMRT.connect((error) => {
    if (error) {
...
app.post('/login', (req, res) => {
    ...
    college_connection.query(query_college, college_code, (err, result_college) => {
        ...
        const query_AMRT = 'SELECT * FROM student WHERE stud_username = ? AND stud_password = ?';
        student_connection_AMRT.query(query_AMRT, [stud_username, stud_password], (err, result_AMRT) => {
            if (err) {
                console.error("Error executing Amrt student query", err);
                return res.status(500).json({ message: "Internal server error" });
            }

            if (result_AMRT.length > 0) {
